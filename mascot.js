...
(function () {
  ...
  var css =
    ...
    '#mascot-bubble{position:absolute;bottom:calc(100% + 15px);right:0;background:none;backdrop-filter:none;-webkit-backdrop-filter:none;border:none;padding:10px 16px;font-size:14px;font-family:"DM Sans",sans-serif;font-weight:900;color:#fff;mix-blend-mode:difference;white-space:nowrap;opacity:0;transform:scale(.7) translateY(12px);transform-origin:bottom right;transition:all .4s cubic-bezier(.34,1.56,.64,1);pointer-events:none;}' +
    ...
    '#gallery-float-btn{position:fixed;bottom:24px;right:96px;z-index:99998;width:60px;height:60px;cursor:pointer;pointer-events:auto;transition:transform .3s cubic-bezier(.34,1.56,.64,1),opacity .3s ease;display:flex;align-items:center;justify-content:center;background:#fff;border-radius:50%;box-shadow:0 10px 24px rgba(0,0,0,.15);border:1.5px solid rgba(0,177,123,.2);opacity:0;pointer-events:none;transform:scale(0.8);}' +
    '#gallery-float-btn.visible{opacity:1;pointer-events:auto;transform:scale(1);}' +
    '#gallery-float-btn:hover{transform:scale(1.15);background:rgba(0,177,123,.05);border-color:rgba(0,177,123,.4);}' +
    '#gallery-float-btn svg{width:28px;height:28px;stroke:#00B17B;}' +
    ...
    '@media(max-width:600px){#mascot-container{right:12px;width:72px;height:72px;bottom:84px;}#mascot-help-btn{right:12px;bottom:16px;width:52px;height:52px;}#gallery-float-btn{right:12px;bottom:76px;width:52px;height:52px;}#mascot-help-panel{right:12px;width:260px;}}';
  ...
  function createMascot() {
    ...
    mascotEl.innerHTML = '<img id="mimg" src="' + CHARS.wave + '" alt="Buddy"/><div id="mascot-bubble"></div>';
  ...
  function showMascot(key, msg, cls, dur) {
    ...
    var img = document.getElementById('mimg'), bub = document.getElementById('mascot-bubble');
    ...
    if (typeof updateGalleryVisibility === 'function') updateGalleryVisibility();
    if (dur > 0) hideTimer = setTimeout(hideMascot, dur);
  }

  function hideMascot() {
    if (!mascotEl) return;
    var bub = document.getElementById('mascot-bubble'); if (bub) bub.classList.remove('show');
  ...
  function buildHelp() {
    ...
    var gbtn = document.createElement('div'); gbtn.id = 'gallery-float-btn'; gbtn.title = 'Open Gallery';
    gbtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>';
    ...
    gbtn.addEventListener('click', function () {
      if (window.openUniversalGallery) window.openUniversalGallery();
      else showMascot('think', 'Gallery coming soon!', 'mascot-float', 3000);
    });

    btn.addEventListener('click', function () {
      helpOpen = !helpOpen;
      if (helpOpen) { refreshPanel(); panel.classList.add('open'); showMascot('wave', 0); }
  ...
  function updateGalleryVisibility() {
    var primary = ['college-student', 'institution', 'junior-student', 'school-inst', 'inst-partners'];
    var btn = document.getElementById('gallery-float-btn');
    if (!btn) return;

    // Check if any gallery tab is active
    var galleryTabs = ['tab-gallery', 'gallery', 'jrtab-gallery', 'schtab-gallery'];
    var isGalleryActive = galleryTabs.some(function (id) {
      var el = document.getElementById(id);
      return el && !el.classList.contains('hidden') && el.style.display !== 'none';
    });

    var isMascotVisible = mascotEl && mascotEl.classList.contains('visible');
    if (primary.indexOf(currentPortal) !== -1 && !isGalleryActive && !isMascotVisible) btn.classList.add('visible');
  ...
  function hookAll() {
    ...
    if (_tab) window.showTab = function (tab) {
      ...
      var m = { courses: ['laptop', 'Explore live classes!', 'mascot-float'], precourses: ['point', 'Learn at your pace!', 'mascot-float'], ats: ['think', 'Check your ATS score!', 'mascot-float'], games: ['cheer', 'Time to play!', 'mascot-bounce'], resources: ['thumbs', 'Great materials here!', 'mascot-float'], about: ['wave', 'Nice to meet you!', 'mascot-wave'], gallery: ['laptop', 'Explore our gallery!', 'mascot-float'] };
    ...
    var _jtab = window.showJrTab;
    if (_jtab) window.showJrTab = function (tab) {
      _jtab.apply(this, arguments);
      setTimeout(updateGalleryVisibility, 100);
      if (tab === 'gallery') showMascot('laptop', 'Explore our gallery!', 'mascot-float', 3000);
    };

    var _stab = window.showSchTab;
    if (_stab) window.showSchTab = function (tab) {
      _stab.apply(this, arguments);
      setTimeout(updateGalleryVisibility, 100);
      if (tab === 'gallery') showMascot('laptop', 'Explore our gallery!', 'mascot-float', 3000);
    };

    var _ug = window.openUniversalGallery;
    if (_ug) window.openUniversalGallery = function () {
      _ug.apply(this, arguments);
      setTimeout(updateGalleryVisibility, 100);
    };
  ...
  ============================================================ */
  function init() {
    createMascot(); buildHelp(); initScroll(); watchEmpty();
    function boot() {
